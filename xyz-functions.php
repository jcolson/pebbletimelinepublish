...
if(!function_exists('xyz_ptp_plugin_get_version'))
{
	function xyz_ptp_plugin_get_version()
	{
		if ( ! function_exists( 'get_plugins' ) )
			require_once( ABSPATH . 'wp-admin/includes/plugin.php' );
		$plugin_folder = get_plugins( '/' . plugin_basename( dirname( XYZ_ptp_PLUGIN_FILE ) ) );
		// 		print_r($plugin_folder);
		return $plugin_folder['pebble-timeline-publish.php']['Version'];
	}
}


if(!function_exists('xyz_ptp_links')){
	function xyz_ptp_links($links, $file) {
		$base = plugin_basename(XYZ_ptp_PLUGIN_FILE);
		if ($file == $base) {

			$links[] = '<a href="http://kb.xyzscripts.com/wordpress-plugins/pebble-timeline-publish/"  title="FAQ">FAQ</a>';
			$links[] = '<a href="http://docs.xyzscripts.com/wordpress-plugins/pebble-timeline-publish/"  title="Read Me">README</a>';
...
if(!function_exists('xyz_ptp_string_limit')){
function xyz_ptp_string_limit($string, $limit) {
...
if(!function_exists('xyz_ptp_getimage')){
function xyz_ptp_getimage($post_ID,$description_org)
{
	$attachmenturl="";
...
if(!function_exists('xyz_ptp_local_date_time')){
	function xyz_ptp_local_date_time($format,$timestamp){
		return date($format, $timestamp + ( get_option( 'gmt_offset' ) * HOUR_IN_SECONDS ));
	}
}

add_filter( 'plugin_row_meta','xyz_ptp_links',10,2);
