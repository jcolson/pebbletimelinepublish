...
class Timeline {
    ...
    private static $ERROR_CODES = array(200 => 'Success!',
    ...
    static function deleteSharedPin($key, $id) {
        ...
        $headers = array('Content-Type: application/json', 'X-API-Key: ' . $key);
        $requestURL = self::$TIMELINE_API . self::$SHARED_PIN_API . $id;
    ...
    private static function sendRequest($url, $method, Array $headers, $postData = null) {
        ...
        $ch = curl_init();
        curl_setopt_array($ch, array(CURLOPT_CUSTOMREQUEST => $method, CURLOPT_RETURNTRANSFER => true, CURLOPT_HTTPHEADER => $headers, CURLOPT_POSTFIELDS => json_encode($postData), CURLOPT_URL => $url, CURLOPT_SSL_VERIFYPEER => false));
        $response = curl_exec($ch);
        $responseInfo = curl_getinfo($ch);
        $RESPONSE_CODE = $responseInfo['http_code'];
